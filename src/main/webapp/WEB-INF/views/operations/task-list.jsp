    ...
    <div class="es-card">
        <div class="card-header">
            ...
            <div style="display:flex;gap:8px;">
                <a href="${fn:escapeXml(pageContext.request.contextPath)}/task/create" class="btn btn-accent btn-sm">+ New Task</a>
            </div>
        </div>

        <!-- Filter -->
        <form method="get" action="${fn:escapeXml(pageContext.request.contextPath)}/task/list" style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap;">
            ...
            <select name="eventId" class="es-select" style="max-width:220px;">
                ...
                <c:forEach var="e" items="${events}">
                    <option value="${fn:escapeXml(e.eventId)}">${fn:escapeXml(e.eventName)}</option>
                </c:forEach>
            </select>
            <button type="submit" class="btn btn-primary btn-sm">Filter</button>
            <a href="${fn:escapeXml(pageContext.request.contextPath)}/task/list" class="btn btn-secondary btn-sm">Clear</a>
        </form>
                    ...
                    <table class="es-table">
                        ...
                        <c:forEach var="t" items="${tasks}" varStatus="st">
                            <tr class="${fn:escapeXml(t.overdue ? 'overdue' : '')}">
                                ...
                                <td><strong>${fn:escapeXml(t.title)}</strong></td>
                                <td>${fn:escapeXml(t.eventName)}</td>
                                <td>${fn:escapeXml(empty t.staffName ? '—' : t.staffName)}</td>
